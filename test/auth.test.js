...
describe('Authentication Service - Verification Code Generation', () => {
  ...
  describe('sendVerificationCode', () => {
    it('should generate and send a 4-digit verification code', async () => {
      ...
      expect(mockMailer.sendMail).toHaveBeenCalledOnce();

      const mailOptions = mockMailer.sendMail.mock.calls[0][0];
      expect(mailOptions.to).toBe(testEmail);
      expect(mailOptions.subject).toBe('登录验证码');
      expect(mailOptions.html).toContain('验证码');

      // Extract code from email HTML
    ...
    it('should enforce rate limiting (1 minute cooldown)', async () => {
      ...
      expect(mockMailer.sendMail).toHaveBeenCalledTimes(1); // Still only 1 call
